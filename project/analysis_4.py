...
class Model(SVC):
    ...
    def fit(self, X, y, sample_weight=None):
        filteredX = X.filter(regex=r'\.mean$')
        self.fit_pca(filteredX)
        self.PCs = self.PCA(filteredX)
        self.y = y
        ...
        eventScaler = StandardScaler(with_mean=True, with_std=True)
        scaledX = pd.DataFrame(eventScaler.fit_transform(X), columns = X.columns)

        eventX = scaledX[y != classMap['nonevent']]
        event_y = y[y != classMap['nonevent']]
        self.event_model = KNN(n_neighbors=7)
        ...
        sfs.fit(eventX, event_y)
        '''
        selected_features = ['partlybad', 'CO2168.mean', 'CO2504.mean', 'H2O336.mean', 'H2O336.std',
       'H2O504.mean', 'H2O672.std', 'NO168.mean', 'NO168.std', 'NO336.mean',
       'NO336.std', 'NO42.mean', 'NO42.std', 'NO504.mean', 'NO504.std',
       'NO672.mean', 'NO672.std', 'NO84.mean', 'NO84.std', 'NOx168.mean',
       'NOx168.std', 'NOx336.mean', 'NOx336.std', 'NOx42.mean', 'NOx42.std',
       'NOx504.mean', 'NOx672.mean', 'NOx672.std', 'NOx84.mean', 'NOx84.std',
       'PTG.mean', 'PTG.std', 'RGlob.mean', 'RHIRGA168.mean', 'RHIRGA336.mean',
       'RHIRGA84.mean', 'RHIRGA84.std', 'SO2168.mean', 'SWS.mean', 'SWS.std',
       'T168.mean', 'T168.std', 'T42.mean', 'T42.std', 'T504.std', 'T672.mean',
       'T672.std', 'T84.std', 'CS.mean', 'CS.std']
        
        #scaledX.columns[sfs.get_support()]

        
        self.event_model.fit(
            eventX,
            event_y
        )
    ...
    def predict(self, X_test, pca=True, return_probs=False):
        ...
        event_preds = self.event_model.predict(X_test)
        class2_preds = super().predict(testPCs)
        preds = np.array([])
        probs = np.array([])

        event_probs = self.event_model.predict_proba(X_test)
...
model = Model(PCA_depth=4).fit(trainDF.drop(columns=['date', 'id', 'class4']), class4)#.predict_and_write(testDF, ids= testDF['id'])
